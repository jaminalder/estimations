...
func TestLobby_Join_RedirectsToRoom_AndLogs(t *testing.T) {
    ...
    req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
    srv.ServeHTTP(rec, req)
    if rec.Code != http.StatusSeeOther { t.Fatalf("create status: %d", rec.Code) }
    loc := rec.Header().Get("Location")
    ...
    req2 := httptest.NewRequest("GET", loc, nil)
    srv.ServeHTTP(rec2, req2)
    if rec2.Code != 200 { t.Fatalf("lobby status: %d", rec2.Code) }
    ...
    if rec4.Code != 200 { t.Fatalf("room status: %d", rec4.Code) }
