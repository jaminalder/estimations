...
type revealRepo struct{ rooms map[domain.RoomID]*domain.Room }
...
func (r *revealRepo) Delete(ctx context.Context, id domain.RoomID) error { delete(r.rooms, id); return nil }

type revealBus struct{ events []any }
func (b *revealBus) Broadcast(ctx context.Context, roomID domain.RoomID, event any) error { b.events = append(b.events, event); return nil }
...
func TestReveal_Success_EmitsOnce(t *testing.T) {
    ...
    if err := room.CastVote(pid, "8"); err != nil { t.Fatalf("cast: %v", err) }

    bus := &revealBus{}
    svc := &Service{Rooms: repo, Bus: bus}

    if err := svc.Reveal(ctx, roomID); err != nil { t.Fatalf("reveal: %v", err) }
    if len(bus.events) != 1 { t.Fatalf("expected 1 event, got %d", len(bus.events)) }
    if _, ok := bus.events[0].(VotesRevealed); !ok { t.Fatalf("wrong event: %T", bus.events[0]) }

    // Idempotent: calling again should not emit a second event
    if err := svc.Reveal(ctx, roomID); err != nil { t.Fatalf("second reveal should be ok: %v", err) }
    if len(bus.events) != 1 { t.Fatalf("expected only 1 event after idempotent reveal, got %d", len(bus.events)) }
}
...
func TestReveal_NoVotes_NoBroadcast(t *testing.T) {
    ...
    if len(bus.events) != 0 { t.Fatalf("no event expected on failure") }
}
