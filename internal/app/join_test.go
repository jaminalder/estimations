...
func TestJoin_Success_BroadcastsEvent(t *testing.T) {
	...
	if err := repo.Create(ctx, room); err != nil {
		t.Fatalf("setup create room: %v", err)
	}
	...
	if pid != "p1" {
		t.Fatalf("participant id mismatch: got %s want p1", pid)
	}
	...
	if room.Participants()[0].Name != "Alice" {
		t.Fatalf("name mismatch: %s", room.Participants()[0].Name)
	}
...
func TestJoin_RoomNotFound(t *testing.T) {
	...
	if len(bus.events) != 0 {
		t.Fatalf("no event should be broadcast on failure")
	}
}
...
func TestJoin_DuplicateName_Fails_NoBroadcast(t *testing.T) {
	...
	if len(bus.events) != 1 {
		t.Fatalf("only first join should broadcast; got %d", len(bus.events))
	}
}
