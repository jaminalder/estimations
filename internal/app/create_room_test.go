...
type repoMem struct{ rooms map[domain.RoomID]*domain.Room }
...
func TestCreateRoom_Basics(t *testing.T) {
    ...
    if err != nil { t.Fatalf("CreateRoom error: %v", err) }
    ...
    if rm.IsRevealed() { t.Fatalf("new room should start in Voting (not revealed)") }
    if rm.RoundIndex() != 0 { t.Fatalf("new room round index should be 0") }
    if len(rm.Participants()) != 0 { t.Fatalf("new room should have no participants") }

    expectedDeck := []string{"0","1","2","3","5","8","13","21","34","?","∞","☕","Pass"}
    deck := rm.Deck()
    if len(deck) != len(expectedDeck) { t.Fatalf("deck length mismatch: got %d want %d", len(deck), len(expectedDeck)) }
    for i := range deck {
